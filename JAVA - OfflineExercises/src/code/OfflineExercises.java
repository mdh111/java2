...
public class OfflineExercises {
	...
	public String getSandwich(String input) {
		...
		int fillingEnd=0;
		int count=0;
		for(i=0;i<input.length()-4 || i<0;i++) {
			if(input.substring(i,i+5).equals("bread") && count==0){
                fillingStart = i+5;
                count+=1;
			}
			if(input.substring(i,i+5).equals("bread") && count==1){
                fillingEnd = i;
                count+=1;
			}
		}
		if(count==2) {
			output=input.substring(fillingStart,fillingEnd);
		}	
